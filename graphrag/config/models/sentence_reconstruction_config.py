...
class SentenceReconstructionConfig(LLMConfig):
    ...
    prompt: str | None = Field(description="The sentence reconstruction prompt to use.", default=None)
    enabled: bool = Field(default=True, description="Whether to reconstruct the sentences.")
    ...
    def resolved_strategy(self, root_dir: str) -> dict:
        ...
        return self.strategy or {
            ...
            **self.parallelization.model_dump(),
            "reconstruction_prompt": (Path(root_dir) / self.prompt).read_bytes().decode(encoding="utf-8")
            if self.prompt
            else None,
